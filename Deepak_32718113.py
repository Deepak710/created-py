    ...
    while True:
        ...
        if len(time_list):
            ...
            if save == 'y' or save == 'Y':
                ...
                ws.write(2, 1, "File Created Time")
                ws.write(2, 2, "Days passed in between")
                for i in range(len(time_list)):
                    ws.write(i+3, 0, file_list[i])
                    ws.write(i+3, 1, ctime(time_list[i]))
                    ago = today - d.datetime.fromtimestamp(time_list[i])
                    if ago.days:
                        ws.write(i+3, 2, "{0} days".format(ago.days))
                    else:
                        ws.write(i+3, 2, "{0} seconds".format(ago.seconds))
